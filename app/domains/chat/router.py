...
def send_real_email(to_email: str, subject: str, body: str):
    """
    Sends an actual email using a Gmail account. 
    NOTE: You must generate an "App Password" in your Google Account Security settings.
    """
    SENDER_EMAIL = "rozthegrey@gmail.com"
    # Replace with your actual 16-character Google App Password
    APP_PASSWORD = "your-google-app-password-here" 

    msg = MIMEMultipart()
    msg['From'] = f"Dunex Support <{SENDER_EMAIL}>"
    msg['To'] = to_email
    msg['Subject'] = subject
...
class BroadcastRequest(BaseModel):
    target_user_id: str  # "all" or a specific user's UUID
    subject: str
    message: str
...
async def create_support_ticket(
    ...
    # 2. Send email to your business email (rozthegrey@gmail.com)
    email_body = f"New Ticket from: {payload.name} ({payload.email})\n\nIssue: {payload.message}"
    background_tasks.add_task(send_real_email, "rozthegrey@gmail.com", f"New Ticket: {payload.subject}", email_body)
    
    # 3. Send confirmation email to the user
    user_body = f"Hello {payload.name},\n\nWe received your ticket: '{payload.subject}'. Our 24/7 team will review it shortly.\n\nYour Message:\n{payload.message}"
    background_tasks.add_task(send_real_email, payload.email or current_user.email, f"Support Received: {payload.subject}", user_body)
...
class BroadcastRequest(BaseModel):
    target_user_id: str  # "all", "custom", or a user's UUID
...
async def admin_broadcast_message(
    ...
    if payload.target_user_id == "all":
        ...
        for email in users:
            background_tasks.add_task(send_real_email, email, payload.subject, payload.message)
        ...
        background_tasks.add_task(send_real_email, payload.custom_email, payload.subject, payload.message)
        ...
        if user:
            background_tasks.add_task(send_real_email, user.email, payload.subject, payload.message)
            return {"status": "success", "recipients": 1}
