...
@router.post("/seed-super-admin", tags=["Development Only"])
async def seed_super_admin(db: AsyncSession = Depends(get_db)):
    """Temporary endpoint to seed the first admin user."""
    admin_email = "admin@dunexmarkets.com"
    admin_password = "SecureAdminPassword123!" # Change this before running!
    ...
    if existing_admin:
        return {"status": "skipped", "message": "Admin already exists."}

    # 2. Create the Admin User
    new_admin = User(
        email=admin_email,
        hashed_password=get_password_hash(admin_password),
        full_name="Dunex Super Admin",
        role="admin",  # 🚨 This is the magic word!
        is_active=True,
        referral_code=generate_referral_code()
    )
    db.add(new_admin)
    await db.flush() # Flushes to get the new_admin.id

    # 3. Provision the Admin Wallet (Required by your system)
    new_wallet = Wallet(
